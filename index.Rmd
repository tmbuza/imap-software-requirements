...
- You can also install snakemake via Mambaforge.
...
It is always a good habit to update conda regularly.
...
### Install snakemake software in activated environment
We already activated the snakemake environment using `conda activate snakemake` command.
...
- Must first create an environment spec file.
- Simply activate the environment you want to reproduce.
- Then run the following command to save the env spec file.

```bash
conda list --explicit > env_spec_file.txt
```

- Then use the spec file when creating a new environment.
```bash
conda create --name new_env --file env_spec_file.txt
```

- Reproducing packages from an existing environment.
```bash
